     ...
     if( j == VERSION )
       {
		...
		ar.READ(j); 
		Data.resize(j);
        for( i = 0; i < j; i++ ) {/* k = CreatPers()*/; ar.READ(Data[i]);}
        //Коллективы
		ar.READ(j);
		Koll.resize(j);
        for( i = 0; i < j; i++ ) { /*k = CreatKoll()*/; ar.READ(Koll[i]); }
        //Группы
		ar.READ(j); 
		Grup.resize(j);
		for( i = 0; i < j; i++ ) {/* k = CreatGrup();*/ ar.READ(Grup[i]); }
        //Дистанции
		ar.READ(j); 
		Dist.resize(j);
		for( i = 0; i < j; i++ ) {/* k = CreatDist();*/ ar.READ(Dist[i]); }
        //Инфо о соревнованиях
        ar.READ(Info);
        //Финиш
		ar.READ(j); 
		Fin.resize(j);
		for( i = 0; i < j; i++ ) {/* k = CreatFin(); */ ar.READ(Fin[i]); }
//         //Si
...
// 
//         //ценность КП
// 		ar.READ(j, sizeof(int)); for( i = 0; i < j; i++ ) ar.read( Cp[i]);
//         //новое содержимое ЧИПов
// 		ar.READ(j, sizeof(int)); for( i = 0; i < j; i++ ) { /*k = CreatSi();*/ ar.read( Si[i]); }
// 
//         readNum();
//         View   = Info.View;
//         POS    = Info.Pos;
...
//             AfxMessageBox("Произведено преобразование в формат WinOrient2 !\nВНИМАНИЕ: после преобразования Вы не сможете открывать этот файл старой программой!\nПроследите, что бы на всех компьютерах была установлена новая версия!");
	      ...
	      //QMessageBox("Неизвестный формат или версия!");
	    }
